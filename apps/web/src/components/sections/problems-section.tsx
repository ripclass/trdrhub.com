...
export function ProblemsSection() {
  return (
    <section id="problems" className="py-16 sm:py-24 md:py-32 bg-slate-900 relative">
      {/* Top border */}
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-10 sm:mb-16">
          <p className="text-red-400 font-semibold mb-3 sm:mb-4 tracking-wide uppercase text-xs sm:text-sm">Problem</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 max-w-6xl mx-auto">
          {problems.map((problem, index) => (
            <div
              key={index}
              className="group relative bg-slate-800/50 rounded-2xl p-5 sm:p-8 hover:bg-slate-800 transition-all duration-300 border border-slate-700/50 hover:border-red-500/30"
            >
              <div className="w-12 sm:w-14 h-12 sm:h-14 bg-red-500/10 rounded-xl flex items-center justify-center mb-4 sm:mb-6 group-hover:bg-red-500/20 transition-colors">
                <problem.icon className="w-6 sm:w-7 h-6 sm:h-7 text-red-400" />
              </div>
