...
_INSPECTION_FACT_FIELDS: Dict[str, Tuple[str, ...]] = {
...
def _normalize_fact_value(field_name: str, value: Any) -> Optional[Any]:
    ...
    if field_name == "inspection_agency":
...
def build_inspection_fact_set(document_payload: Dict[str, Any]) -> Dict[str, Any]:
    payload = document_payload or {}
    fields = _source_fields(payload)
    details = _detail_map(payload)
    facts = []

    for field_name, aliases in _INSPECTION_FACT_FIELDS.items():
    ...
    document_type = str(payload.get("document_type") or "inspection_certificate").strip() or "inspection_certificate"
    document_subtype = str(payload.get("inspection_subtype") or document_type).strip() or None
