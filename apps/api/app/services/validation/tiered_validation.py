...
def _build_opus_veto_prompt(
    ...
    return (
        f"Full presentation set under review (primary document type: {document_type}). "
        "All document types listed below are IN SCOPE for this review — findings "
        "may span any combination of them.\n\n"
        f"{doc_sections}\n\n"
        ...
        "certificate of origin, bill of lading, etc. rather than the primary doc type.\n"
        "- Be conservative: only drop a finding if you can point to specific evidence "
        "in the document sections above that contradicts it.\n"
        ...
        "- If a finding cites a missing field/signature/notation, CHECK the corresponding "
        "document section above. Only drop if the section clearly shows the field/signature/notation "
        "is present under a different name. Do NOT drop just because the rule name mentions "
        "a path that isn't in the document — confirm what's actually there vs what the LC "
        "requires.\n\n"
        "Return JSON only, no prose."
    )
