...
def upgrade():
    ...
    # Create AddressType enum (only if it doesn't exist)
    result = bind.execute(sa.text(
        "SELECT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'addresstype')"
    )).scalar()
    if not result:
        op.execute("CREATE TYPE addresstype AS ENUM ('business', 'shipping', 'billing', 'warehouse', 'custom')")
    
    # Create ComplianceStatus enum (only if it doesn't exist)
    result = bind.execute(sa.text(
        "SELECT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'compliancestatus')"
    )).scalar()
    if not result:
    ...
    # Create company_addresses table (only if it doesn't exist)
    ...
    # Create company_compliance_info table (only if it doesn't exist)
    if 'company_compliance_info' not in existing_tables:
        op.create_table(
    ...
    # Create default_consignee_shipper table (only if it doesn't exist)
    if 'default_consignee_shipper' not in existing_tables:
        op.create_table(
