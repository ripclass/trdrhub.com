...
- `alembic/versions/20260510_pricing_restructure_tier.py` — drops `ck_companies_tier_valid` (the pre-existing CHECK that only allowed `('solo','sme','enterprise')`) → rewrites `companies.tier` `'sme'` → `'business'` → re-creates the CHECK with the 7 new values → changes column default `'sme'` → `'payg'`; logs the `'sme'` row count first. **✅ Applied on Render trdrhub-api 2026-05-10 13:52 UTC** (commit `5b665fac`; the first run without the constraint-swap hit a CheckViolation and rolled back). 141 rows migrated `sme`→`business`; `/health/db-schema` ok. Future migrations still need a manual `render jobs create srv-d41dio8dl3ps73db8gpg --start-command "alembic upgrade head"` (`reference_render_migrations`).
